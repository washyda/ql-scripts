...
2. 任务开头先写无尾逗号的 `// name: "任务名称"` 与 `// cron "..." script-path=...` 行，再写 `// @name`、`// @description`、`// @cron`。订阅器按首次出现的 `name` 与 `cron` 解析，不能让 `@name`、`@cron` 排在前面；其后紧跟中文使用说明注释（含用途、环境变量与多账号配置说明）。
...
- 多账号任务应隔离每个账号的上下文和错误。
- 本仓库青龙订阅器使用五段 cron：分、时、日、月、周。
- 默认每日任务使用 `12 8 * * *`（每天 08:12），并可通过 `src/core/task.ts` 的共享延迟工具额外随机等待 1–30 秒以错开秒级请求；不要使用非标准的 `~` Cron 语法或范围表达式来模拟随机时间，后者会重复执行。
- 不编译或提交 `.js` 任务产物。
